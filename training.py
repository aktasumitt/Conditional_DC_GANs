...
def Training(EPOCHS,resume_epoch,BATCH_SIZE,F_DIM,gradient_penalty_fn,LAMDA_GP,Img_dataloader,Discriminator_Model,Generator_Model,Optim_Gen,Optim_Disc,Save_Checkpoints,CALLBACK_PATH,tensorboard,devices):
    ...
    for epoch in range(resume_epoch,EPOCHS):
        ...
        for batch,(img,label) in enumerate(Img_dataloader):

            real_img=img.to(devices)
            labels=label.to(devices)
            ...
            for _ in range(5):
                ...
                fake_img=Generator_Model(noise_,labels)
                out_fake=Discriminator_Model(fake_img,labels).reshape(-1)
                out_real=Discriminator_Model(real_img,labels).reshape(-1)
